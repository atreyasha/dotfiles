...
****** find way to automatically reparse document with =reftex-parse-all= when building document -> will ensure labels are updated for autocompletion as they are currently not refreshed -> can be added to =company-reftex=
...
**** Long-term
***** emacs
****** fix slow response with =ipython= inferior shell
