...
******* first step for writing to common history file -> zsh: =setopt shared_history=, bash: extra prompt commands ~PROMPT_COMMAND='history -a; history -c; history -r'~
...
** Completed
***** DONE i3: make all audio channels muted at boot (now only main channel is muted) -> very complicated because of pulseaudio and alsa simultaneous actions: https://github.com/alsa-project/alsa-utils/issues/72
      CLOSED: [2020-11-24 Tue 14:37]
****** switch to single volume value between different channels -> will solve all of these problems altogether -> still challenging to solve
