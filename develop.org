...
***** TODO find workflow for shfmt on all shell scripts including zsh -> use this as a git hook and replace all tabs with 2 spaces for uniformity esp. in zsh script -> port this to git hooks where it could be useful -> add another git hook to ensure all shell tildes are replaced with $HOME
...
***** clean out vimrcs and perhaps simply load a new vimrc to overwrite git upstream defaults
...
***** *untested:* perhaps try to unset dpms before sleep -> leads to problems waking from sleep
**** early KMS
