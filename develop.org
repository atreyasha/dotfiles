...
****** TODO find way to automatically reparse document with ~reftex-parse-all~ when building document -> will ensure labels are updated for autocompletion as they are currently not refreshed -> can be added to ~company-reftex~ directly in a new branch since permission to push is granted
