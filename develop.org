...
** Tasks
*** Dotfiles
***** TODO fine tune installation script for user and root level dotfiles
***** find ways to reduce hard-coding of configs/dotfiles to `/home/shankar` or `~` -> perhaps with makefile
***** bash/zsh fix reduced hard-coding -> add if statements to check for certain commands to reduce hard-coding
