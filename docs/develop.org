...
****** this will fix issue of child directories being folded at a parent-level inadvertently, which might be problematic
****** all fold scripts must be updated to reflect across all repositories
****** add ~vim-syntax-clingo~ to folded framework
