...
****** ~vim~ and ~pip~ can be done easily via git submodules and ~pip~ CLI
****** ~emacs~ would require submodules update and spacemacs package updates as well
...
****** add ~test~ commands for all repositories for sanity checks during system updates
