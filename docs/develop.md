...
1.  **TODO** Private
    ...
    2.  abook -\> figure out how to handle this as it gets overwritten

        1.  maybe can have different workflows for directories that
    ...
    5.  mbsyncrc, msmtp and individual account muttrcs

        1.  need to copy account and breaker pseudo-mailboxes

        2.  remove autogenerated comments later on when tracking

    6.  moves non-private user-specific dotfiles to privates repository

        1.  (sub)keys are present in three cases: git, neomutt, pass,
            signature and autorandr (too)
